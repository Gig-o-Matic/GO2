...
def get_model_from_URL(node_name):
    ...
    model_alias = MODEL_NAME_ALIAS[node_name]
...
class Endpoint(BaseHandler):

    @rest_user_required
    @CSOR_Jsonify
    def get(self,query="",endpoint=None, *args, **kwargs):
        handlers = {
            "agenda" : agenda.RestAgendaGet,
            "plan" : plan.RestPlanGet,
        }
        try:
            return handlers[endpoint](self, *args, **kwargs)
    ...
    @rest_user_required
    @CSOR_Jsonify
    def post(self,query="",endpoint=None, *args, **kwargs):
        handlers = {
            "logout" : login.RestLogout
        }
        try:
            return handlers[endpoint](self, *args, **kwargs)
        except webapp2.HTTPException:
    ...
    @rest_user_required
    @CSOR_Jsonify
    def put(self,query="",endpoint=None, *args, **kwargs):
        handlers = {
            "plan" : plan.RestPlanPut
        }
        try:
            return handlers[endpoint](the_handler=self, *args, **kwargs)
        except webapp2.HTTPException:
            raise
        except:
            self.abort(404)
...
class ReST(BaseHandler):
    ...
    def get(self,query=""):
        ...
        if qry :
            ...
            if node[2] in MODEL_EXPOSE.keys():
                dataObject=Object_by_id.to_dict(include=MODEL_EXPOSE[node[2]])
            else:
